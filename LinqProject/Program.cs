...
namespace LinqProject
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            //Algotirmik listeleme/filtreleme
            ...
            // Linq kullanılarak listeleme/filtreleme
            var result = products.Where(p => p.UnitPrice > 5000 && p.UnitsInStock > 3); //p yerine product da yazılabilir.
        ...
        // Link olmadan fonksiyon şeklinde gösterim
        static List<Product> GetProducts(List<Product> products)
        {
            List<Product> filterdProduct = new List<Product>();
            foreach (var product in products)
            {
                if (product.UnitPrice > 5000 && product.UnitsInStock > 3)
                {
                    filterdProduct.Add(product);
                }
            }
            return filterdProduct;
        }

        // Link ile fonksiyon şeklinde gösterim
